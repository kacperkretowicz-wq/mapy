...
System umożliwia wizualizację punktów na mapie, filtrowanie handlowców, zaawansowane wyszukiwanie, a także interaktywną zmianę statusu wizyt i notatek bezpośrednio z poziomu smartfona.
...
1. **Zarządzanie Użytkownikami (RLS)**: W nagłówku aplikacji możesz wybrać zalogowanego handlowca (np. `jan.kowalski@firma.pl`). Zaznaczenie opcji **"Tylko moje stacje"** sprawi, że na mapie i liście pojawią się wyłącznie punkty przypisane do danej osoby.
2. **Interaktywna Mapa**:
   ...
   - Kliknięcie **Zapisz zmiany** automatycznie zaktualizuje mapę i zapisze dane trwale w pamięci przeglądarki telefonu (`localStorage`). Dane nie znikną po odświeżeniu strony!
3. **Widok Listy i GPS**:
   ...
   - Możesz symulować pozycję GPS, wybierając różne miasta z listy (np. Warszawa, Wrocław) lub wybrać **"Urządzenie (Real GPS)"**, by pobrać rzeczywiste współrzędne z telefonu!
